...
export function cleanCanvasBizConfigRefs(cfg: Record<string, unknown>, deletedIds: Set<string>): BizConfig {
  ...
  const cleanRoutedItems = (items: unknown) => {
    if (!Array.isArray(items)) return undefined
    return items.map((item) => {
      const routedItem = item as Record<string, unknown>
      return {
        ...routedItem,
        nextNodeId: clean(routedItem.nextNodeId) as string | undefined,
      }
    })
  }
  ...
  return {
    ...
    priorities: cleanRoutedItems(biz.priorities),
    groups: cleanRoutedItems(biz.groups),
    paths: cleanRoutedItems(biz.paths),
    variants: cleanRoutedItems(biz.variants),
    bands: cleanRoutedItems(biz.bands),
  }
}
