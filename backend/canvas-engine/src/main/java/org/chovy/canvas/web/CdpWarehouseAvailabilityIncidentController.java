...
/**
 * CdpWarehouseAvailabilityIncidentController 暴露 web 场景的 HTTP 接口。
 */
@RestController
@RequestMapping("/warehouse/availability/incidents")
public class CdpWarehouseAvailabilityIncidentController {
    ...
    /**
     * 创建 CdpWarehouseAvailabilityIncidentController 实例并注入 web 场景依赖。
     * @param incidentService 依赖组件，用于完成数据访问或外部能力调用。
     */
    public CdpWarehouseAvailabilityIncidentController(
    ...
    /**
     * 扫描 CDP 数仓 Availability Incident风险接口，对应 POST /scan。
     ...
     *
     * @param req 请求体，可选。
     * @return 异步返回统一响应，包含扫描 CDP 数仓 Availability Incident风险后的业务数据。
     */
    @PostMapping("/scan")
    public Mono<R<CdpWarehouseAvailabilityIncidentService.ScanResult>> scan(
