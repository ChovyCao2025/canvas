...
/**
 * CdpWarehouseReadinessIncidentController 暴露 web 场景的 HTTP 接口。
 */
@RestController
...
public class CdpWarehouseReadinessIncidentController {
    ...
    /**
     * 创建 CdpWarehouseReadinessIncidentController 实例并注入 web 场景依赖。
    ...
    /**
     * 扫描 CDP 数仓 Readiness Incident风险接口，对应 POST /scan。
     * 接口先解析当前租户上下文，按租户隔离处理数据。
     * 主要委托 incidentService.scan 完成业务处理。
     * 副作用由下游服务封装，通常会写入状态、审计或任务记录。
     * 阻塞型服务调用被包在 Mono 中，并调度到 boundedElastic 线程池执行。
     *
     * @return 异步返回统一响应，包含扫描 CDP 数仓 Readiness Incident风险后的业务数据。
     */
    @PostMapping("/scan")
