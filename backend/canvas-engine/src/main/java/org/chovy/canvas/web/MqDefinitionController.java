...
import org.chovy.canvas.infrastructure.redis.MqRouteRefreshService;
...
public class MqDefinitionController {
    ...
    /** 分页查询 MQ 消息定义。 */
    @GetMapping
    public Mono<R<PageResult<MqMessageDefinitionDO>>> list(
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "20") int size,
            @RequestParam(required = false) Integer enabled) {
        return Mono.fromCallable(() -> {
            // enabled 为空时不过滤，返回全部记录
            var wrapper = new LambdaQueryWrapper<MqMessageDefinitionDO>()
                    .eq(enabled != null, MqMessageDefinitionDO::getEnabled, enabled)
                    .orderByAsc(MqMessageDefinitionDO::getId);
            Page<MqMessageDefinitionDO> p = mapper.selectPage(new Page<>(page, size), wrapper);
    ...
    /** 创建 MQ 消息定义。 */
    @PostMapping
    public Mono<R<MqMessageDefinitionDO>> create(@RequestBody MqMessageDefinitionDO body) {
        return Mono.fromCallable(() -> {
                    mapper.insert(body);
    ...
    /** 更新 MQ 消息定义。 */
    @PutMapping("/{id}")
    public Mono<R<Void>> update(@PathVariable Long id, @RequestBody MqMessageDefinitionDO body) {
