...
/**
 * CdpWarehouseReadinessController 暴露 web 场景的 HTTP 接口。
 */
@RestController
...
public class CdpWarehouseReadinessController {
    ...
    /**
     * 创建 CdpWarehouseReadinessController 实例并注入 web 场景依赖。
    ...
    /**
     * 查询 CDP 数仓 Readiness就绪度接口，对应 GET /readiness。
     * 接口先解析当前租户上下文，按租户隔离读取数据。
     * 主要委托 readinessService.readiness 完成业务处理。
     * 该接口只读取数据，不主动触发业务写入。
     * 阻塞型服务调用被包在 Mono 中，并调度到 boundedElastic 线程池执行。
     *
     * @return 异步返回统一响应，包含查询 CDP 数仓 Readiness就绪度后的业务数据。
     */
