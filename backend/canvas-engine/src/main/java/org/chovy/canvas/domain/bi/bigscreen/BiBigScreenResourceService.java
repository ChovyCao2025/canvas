...
/**
 * BiBigScreenResourceService 编排 domain.bi.bigscreen 场景的领域业务规则。
 */
@Service
public class BiBigScreenResourceService {
    ...
    /**
     * 创建 BiBigScreenResourceService 实例并注入 domain.bi.bigscreen 场景依赖。
     * @param workspaceMapper 依赖组件，用于完成数据访问或外部能力调用。
     * @param screenMapper 依赖组件，用于完成数据访问或外部能力调用。
     ...
     *
     * @param tenantId 租户标识，用于限定 BI 资源、权限和审计数据的隔离范围
     * @param screenKey 大屏业务键，用于定位大屏草稿、发布版本和运行资源
    ...
    public BiBigScreenResource restoreVersion(Long tenantId, String username, String screenKey, int version) {
        // 大屏恢复以当前租户和固定 BI 工作区为边界，读取指定发布快照后重新保存为草稿。
        // 此 Service 暂无独立协作锁/审批 guard，恢复沿用 saveDraft 的资源键和租户隔离校验。
        Long scopedTenantId = normalizeTenant(tenantId);
        Long workspaceId = workspaceId(scopedTenantId);
    ...
    private BiBigScreenVersionView toVersionView(BiBigScreenVersionDO row) {
        // 版本视图展示发布时保存的完整资源 JSON，保证列表查看与恢复使用同一份快照内容。
    ...
    private void insertVersion(Long tenantId,
                               ...
                               String username) {
        // 发布版本以完整大屏资源为快照，包含画布尺寸、背景、布局、刷新和移动端布局等运行态口径。
        // 快照绑定租户、工作区和大屏 id，发布人缺省为 system 以便审计追溯。
    ...
    /**
     * 执行 emptyObject 流程，围绕 empty object 完成校验、计算或结果组装。
     *
     * @param json JSON 字符串，承载结构化配置或明细。
     * @return 返回 empty object 生成的文本或业务键。
     */
