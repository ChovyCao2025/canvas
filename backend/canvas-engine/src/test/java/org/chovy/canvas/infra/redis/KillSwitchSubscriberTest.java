...
class KillSwitchSubscriberTest {
    ...
    void shutdownDisposesManagedRedisSubscriptionAndContainer() {
        ...
        ReactiveRedisMessageListenerContainer container = mock(ReactiveRedisMessageListenerContainer.class);
        when(subscription.isDisposed()).thenReturn(false);
        ReflectionTestUtils.setField(subscriber, "subscription", subscription);
        ReflectionTestUtils.setField(subscriber, "listenerContainer", container);
