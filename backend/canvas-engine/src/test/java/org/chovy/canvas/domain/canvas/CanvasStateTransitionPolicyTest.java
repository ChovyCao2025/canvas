...
class CanvasStateTransitionPolicyTest {
    ...
    @Test
    void publishedRuntimeMutationRequiresCurrentlyPublishedCanvas() {
        assertThatCode(() -> policy.assertPublishedRuntimeMutationAllowed(canvas(CanvasStatusEnum.PUBLISHED)))
                .doesNotThrowAnyException();
        assertThatThrownBy(() -> policy.assertPublishedRuntimeMutationAllowed(canvas(CanvasStatusEnum.DRAFT)))
                .isInstanceOf(IllegalStateException.class)
                .hasMessageContaining("DRAFT");
        assertThatThrownBy(() -> policy.assertPublishedRuntimeMutationAllowed(canvas(CanvasStatusEnum.OFFLINE)))
                .isInstanceOf(IllegalStateException.class)
                .hasMessageContaining("OFFLINE");
        assertThatThrownBy(() -> policy.assertPublishedRuntimeMutationAllowed(canvas(CanvasStatusEnum.KILLED)))
                .isInstanceOf(IllegalStateException.class)
                .hasMessageContaining("KILLED");
        assertThatThrownBy(() -> policy.assertPublishedRuntimeMutationAllowed(canvas(CanvasStatusEnum.ARCHIVED)))
                .isInstanceOf(IllegalStateException.class)
