-- Ensure legacy single-tenant rows have a stable tenant before NOT NULL constraints are enforced.
...
-- Attach orphaned principal and canvas rows to the default tenant introduced above.
UPDATE sys_user
...
-- Execution traces inherit tenant scope from their execution to keep audit queries tenant-safe.
...
-- Lock core tenant columns after all historical rows have been backfilled.
