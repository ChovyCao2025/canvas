...
**⚠️ 实现注意：禁止使用 `KEYS` 命令检测路由表是否为空**

`isRouteTableEmpty()` 不得使用 `redis.keys("canvas:trigger:*")`——该命令是 O(N)，遍历 Redis 全库，生产环境 key 数量多时会造成毫秒级甚至秒级阻塞（Redis 单线程）。

应改用 `SCAN` 游标命令（O(1) 每次调用，分批次扫描，不阻塞）：

```java
ScanOptions opts = ScanOptions.scanOptions().match("canvas:trigger:*").count(1).build();
try (var cursor = redisConnectionFactory.getConnection().scan(opts)) {
    return !cursor.hasNext();  // 找到一条即停止，不全量扫描
}
```
...
> ⚠️ **时序约束（实现关键）**：SKIPPED 写入分两个阶段，必须严格区分：
>
> **阶段一（执行中，立即写入）**：当 IF判断 / SELECTOR 等分支节点确定走某条路后，必须**立即**将未走的分支入口节点标记为 SKIPPED（写入 `ctx.nodeStatuses`）。
> 目的：让后续 LOGIC_RELATION(AND) 节点能在执行中即时检测到"上游已 SKIPPED"，并立刻失败，而不是永久等待一个永远不会来的 SUCCESS。
>
> **阶段二（执行结束后，批量写入）**：整个 DAG 执行完毕后，扫描所有节点，对从未进入 `nodeStatuses` 的节点批量写入 SKIPPED 轨迹记录（写入 DB），用于前端可视化展示。
>
> 两阶段分工：阶段一影响执行逻辑，阶段二只影响轨迹记录。

以下是典型的 SKIPPED 场景（举例，非穷举）：

| 场景 | 被跳过的节点 | 写入阶段 |
|------|------------|---------|
| IF判断走 successNodeId | failNodeId 指向的分支入口节点 | **阶段一**（立即，影响 LOGIC_RELATION 检测） |
| 条件选择器命中第一个分支 | 其他分支入口节点 | **阶段一**（立即） |
| 优先级节点子节点1成功 | 子节点2、3... 入口节点 | 阶段二（批量） |
| LOGIC_RELATION AND模式，上游失败，整体 FAILED | nextNodeId 及所有后续节点 | 阶段二（批量） |

**本质**：阶段一保证执行逻辑正确；阶段二保证轨迹数据完整。两阶段互补，缺一不可。
...
`/auth/login` 是公开接口，若无限制则攻击者可无限次尝试密码（字典攻击）。

**方案：Redis 失败计数 + 账号临时锁定**

```
Key: canvas:login:fail:{username}
Value: 失败次数（Integer）
TTL: 15 分钟（每次失败后重置 TTL）

Key: canvas:login:locked:{username}
Value: "1"
TTL: 15 分钟

规则：
  1. 登录失败：INCR canvas:login:fail:{username}，并设置 TTL=15min
  2. 失败次数 ≥ 5：SET canvas:login:locked:{username} EX 900（15min）
  3. 任何登录请求前：先检查 locked key，存在则返回 429 "账号已锁定，请 15 分钟后重试"
  4. 登录成功：DEL canvas:login:fail:{username}（清除计数）
```
...
| 情形 | HTTP 状态码 | 错误码 |
|------|------------|--------|
| 账号已锁定 | 429 Too Many Requests | `AUTH_004` |
| 密码错误（未锁定） | 401 Unauthorized | `AUTH_001` |
| 用户已禁用 | 403 Forbidden | `AUTH_005` |

**为什么用 Redis 而非数据库**：锁定检查在登录鉴权最前置，需要毫秒级响应，Redis O(1) 更合适。TTL 天然支持"N 分钟后自动解锁"，无需额外 Job 清理。
...
`POST /canvas/execution/{id}/approve|reject` 接口必须校验当前登录用户是否在该审批记录的 `approvers` 列表中，否则任意 ADMIN 都可强制通过他人的审批，绕过业务风控。

```
校验规则：
  1. 从 JWT 中提取当前用户的 username / userId
  2. 查询 canvas_manual_approval.approvers（JSON 数组）
  3. 若当前用户不在列表中 → 返回 403 FORBIDDEN
  4. 允许系统超级管理员（或专用服务账号）绕过此限制（Watchdog 超时处理）
```
