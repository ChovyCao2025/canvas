...
本目录是容量压测的唯一有效入口。单机本地压测按 [本地容量压测操作手册](./local-capacity-runbook.md) 操作；多台压测机或多后端节点按 [多机容量压测操作手册](./distributed-capacity-runbook.md) 操作。所有路径都必须使用 `tools/perf/perf-guide.mjs` 做正确性闸门。

不要把历史归档中的容量估算当成实测结果。一个容量数字只有同时具备以下证据时才有效：

- 唯一的 `perfRunId`
- runner summary JSON
- 多机压测的 distributed summary JSON
...
通过后，单机按完整流程执行 `fixture`、`smoke`、`accuracy`、`threshold`、`soak`、`report`、`cleanup`。多机必须先完成单机 `fixture`、`smoke`、`accuracy`，再执行 `distributed-plan`、`distributed-worker`、`distributed-report`：

- [本地容量压测操作手册](./local-capacity-runbook.md)
- [多机容量压测操作手册](./distributed-capacity-runbook.md)
...
- 如果 `distributed-report` 不是 `PASS`，不得发布多机吞吐、QPS、p95、p99 或容量估算。
