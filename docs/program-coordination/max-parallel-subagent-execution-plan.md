...
| OSG-C07 | No, coordinator-owned | Plugin registry extension point decision | final owner split: `canvas-platform` registry metadata/enablement, `canvas-context-execution` handler binding/node metadata/runtime validation, old `PluginRegistryService`/`JdbcPluginRepository`/`HandlerRegistry` as legacy bridge inputs only | DDD canvas/execution and platform ownership explicit | Sequential bottleneck |
...
| P7 plugin burst | DDD-W08 integrated and OSG-C07 done | 6 | 2 | 8 | Six official plugin workers can run in parallel after G10 evidence if each owns a separate plugin package and docs file. |
