...
`docs/architecture/archive/completed/specs/P3-02-service-decomposition-and-domain-boundaries-spec.md` requires a reversible service or bounded-context split. `docs/architecture/adr/ADR-0006-service-extraction-gate.md` blocks physical service extraction until data ownership, API contracts, event contracts, tenant propagation, idempotency, observability, rollout, rollback, and characterization tests are proven.
