...
Keep MySQL as the system of record, Redis as bounded runtime coordination/cache state, and RocketMQ as asynchronous delivery transport. Redis key families must be cataloged in `docs/architecture/decisions/reference/redis-key-catalog.md`; high-volume MySQL tables must follow the P2-02 retention policy; RocketMQ consumers must have retry, DLQ, and replay runbooks.
