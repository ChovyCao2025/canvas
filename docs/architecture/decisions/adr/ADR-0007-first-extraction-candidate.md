...
`docs/architecture/archive/completed/specs/P3-02-service-decomposition-and-domain-boundaries-spec.md` requires a reversible service or bounded-context split. `docs/architecture/decisions/adr/ADR-0006-service-extraction-gate.md` blocks physical service extraction until data ownership, API contracts, event contracts, tenant propagation, idempotency, observability, rollout, rollback, and characterization tests are proven.
...
- use `docs/architecture/evidence/runbooks/deploy-rollback.md` for application rollback if release health or SLO checks fail.
...
- `docs/architecture/decisions/work-products/p3-02-service-boundaries/domain-contract-inventory.md` has current REST, event, Redis key, MQ, table, DTO, tenant, compatibility window, rollback trigger, and reconciliation details.
...
- `docs/architecture/decisions/adr/ADR-0006-service-extraction-gate.md`
