# V339 creator provider mutation gateway

Migration: `V339__creator_provider_mutation_gateway.sql`

Touched tables identified from the migration SQL:

- `creator_provider_mutation`

High-risk statements requiring release evidence:

- `UNIQUE KEY \`uk_creator_provider_mutation_key\` (\`tenant_id\`, \`mutation_key\`),`
- `UNIQUE KEY \`uk_creator_provider_mutation_idempotency\` (\`tenant_id\`, \`provider\`, \`idempotency_key\`),`
...
export MIGRATION_ID=V339__creator_provider_mutation_gateway
export TABLES="creator_provider_mutation"
