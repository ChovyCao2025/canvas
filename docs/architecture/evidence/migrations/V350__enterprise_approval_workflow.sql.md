# V350 enterprise approval workflow

Migration: `V350__enterprise_approval_workflow.sql`
...
export MIGRATION_ID=V350__enterprise_approval_workflow
export TABLES="approval_audit_event approval_definition approval_instance approval_task"
...
If application writes have occurred after the migration, do not restore blindly. Stop writers, export post-migration deltas, and get Approval platform owner plus DBA approval for a merge or point-in-time restore.
...
Approval platform owner owns the functional rollback decision. Runtime platform owns Flyway execution ordering. DBA approval is required before restoring data or editing `flyway_schema_history`.
