...
| --- | --- | --- | --- |
| audit evidence | Compliance owner | `AuditEventService`, `canvas_audit_log`, `docs/architecture/evidence/compliance/audit-event-matrix.md`, controller mutation hooks | `mvn test -pl canvas-engine -Dtest=AuditEventServiceTest,CanvasControllerOperatorLoopTest,DataSourceConfigControllerTest` |
...
| retention evidence | Runtime platform | `docs/architecture/evidence/capacity/retention-policy.md`, retention run tables | Review retention run id, cutoff, status, archive target |
