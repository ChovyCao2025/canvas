...
**Goal:** Add audience set operations, snapshots, freshness, health monitoring, user 360 refinements, data catalog, and quality checks.
...
Create `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/audience/AudienceOperationsServiceTest.java` with tests for authorization, tenant scoping, and the main success path named after `audience-operations-data-quality`. Use existing controller or service tests in `backend/canvas-engine/src/test/java/org/chovy/canvas` as style references.

- [ ] **Step 2: Run backend contract tests and confirm red state**

Run: `cd backend && mvn -pl canvas-engine test -Dtest=AudienceOperationsServiceTest`
...
Create `frontend/src/pages/audience-list/audienceOperations.test.tsx` with Vitest coverage for loading, empty, success, permission, and server-error states for the first UI slice.

- [ ] **Step 4: Run frontend workflow tests and confirm red state**

Run: `cd frontend && npm test -- audienceOperations.test.tsx`
...
- `frontend/src/pages/audience-list/index.tsx`
- `frontend/src/pages/cdp-user-detail/index.tsx`
- Test: `frontend/src/pages/audience-list/audienceOperations.test.tsx`
...
Run: `git add backend/canvas-engine/src frontend/src docs/product-evolution/specs docs/product-evolution/plans && git commit -m "feat: implement audience-operations-data-quality slice"`
