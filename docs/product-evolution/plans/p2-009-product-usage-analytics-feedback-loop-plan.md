...
**Goal:** Track product usage, collect feedback, manage feature flags, and define product alert rules.
...
Create `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/productanalytics/ProductAnalyticsServiceTest.java` with tests for authorization, tenant scoping, and the main success path named after `product-usage-analytics-feedback-loop`. Use existing controller or service tests in `backend/canvas-engine/src/test/java/org/chovy/canvas` as style references.

- [ ] **Step 2: Run backend contract tests and confirm red state**

Run: `cd backend && mvn -pl canvas-engine test -Dtest=ProductAnalyticsServiceTest`
...
Create `frontend/src/services/productAnalytics.test.ts` with Vitest coverage for loading, empty, success, permission, and server-error states for the first UI slice.

- [ ] **Step 4: Run frontend workflow tests and confirm red state**

Run: `cd frontend && npm test -- productAnalytics.test.ts`
...
- `frontend/src/services/productAnalytics.ts`
- `frontend/src/components/feedback/FeedbackButton.tsx`
- Test: `frontend/src/services/productAnalytics.test.ts`
...
Run: `git add backend/canvas-engine/src frontend/src docs/product-evolution/specs docs/product-evolution/plans && git commit -m "feat: implement product-usage-analytics-feedback-loop slice"`
