...
**Goal:** Create a deeper analytics command center with revenue, ROI, channel comparison, canvas comparison, report templates, scheduled reports, export, and chart drill-down.
...
**Data And Config**
- `backend/canvas-engine/src/main/resources/db/migration/V101__analytics_command_center.sql`
...
**Files:**
- Create or modify: `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/analytics/AnalyticsCommandCenterServiceTest.java`
- Create or modify: `frontend/src/pages/analytics-command-center/analyticsCommandCenter.test.tsx`
- Read: `docs/product-evolution/specs/p2-007-analytics-command-center.md`

- [ ] **Step 1: Write backend contract tests**

Create `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/analytics/AnalyticsCommandCenterServiceTest.java` with tests for authorization, tenant scoping, and the main success path named after `analytics-command-center`. Use existing controller or service tests in `backend/canvas-engine/src/test/java/org/chovy/canvas` as style references.

- [ ] **Step 2: Run backend contract tests and confirm red state**

Run: `cd backend && mvn -pl canvas-engine test -Dtest=AnalyticsCommandCenterServiceTest`
...
Create `frontend/src/pages/analytics-command-center/analyticsCommandCenter.test.tsx` with Vitest coverage for loading, empty, success, permission, and server-error states for the first UI slice.

- [ ] **Step 4: Run frontend workflow tests and confirm red state**

Run: `cd frontend && npm test -- analyticsCommandCenter.test.tsx`
...
- `backend/canvas-engine/src/main/java/org/chovy/canvas/domain/analytics/AnalyticsCommandCenterService.java`
- `backend/canvas-engine/src/main/resources/db/migration/V101__analytics_command_center.sql`
- Test: `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/analytics/AnalyticsCommandCenterServiceTest.java`
...
- `frontend/src/pages/analytics-command-center/index.tsx`
- `frontend/src/services/analyticsApi.ts`
- Test: `frontend/src/pages/analytics-command-center/analyticsCommandCenter.test.tsx`
...
Run: `git add backend/canvas-engine/src frontend/src docs/product-evolution/specs docs/product-evolution/plans && git commit -m "feat: implement analytics-command-center slice"`
