...
**Goal:** Provide demo canvases, mock data, sandbox tenants, reset/expiry behavior, and sales enablement material.
...
Create `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/demo/DemoSandboxServiceTest.java` with tests for authorization, tenant scoping, and the main success path named after `sandbox-demo-sales-enablement`. Use existing controller or service tests in `backend/canvas-engine/src/test/java/org/chovy/canvas` as style references.

- [ ] **Step 2: Run backend contract tests and confirm red state**

Run: `cd backend && mvn -pl canvas-engine test -Dtest=DemoSandboxServiceTest`
...
Create `frontend/src/pages/demo-sandbox/demoSandbox.test.tsx` with Vitest coverage for loading, empty, success, permission, and server-error states for the first UI slice.

- [ ] **Step 4: Run frontend workflow tests and confirm red state**

Run: `cd frontend && npm test -- demoSandbox.test.tsx`
...
- `frontend/src/pages/demo-sandbox/index.tsx`
- `frontend/src/services/demoSandboxApi.ts`
- Test: `frontend/src/pages/demo-sandbox/demoSandbox.test.tsx`
...
Run: `git add backend/canvas-engine/src frontend/src docs/product-evolution/specs docs/product-evolution/plans && git commit -m "feat: implement sandbox-demo-sales-enablement slice"`
