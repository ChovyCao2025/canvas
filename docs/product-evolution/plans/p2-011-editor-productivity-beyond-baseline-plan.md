...
**Goal:** Improve large-canvas productivity with shortcuts, node search, batch operations, context menu, breadcrumbs, recent/favorites, field help, skeleton loading, rich inputs, async validation, auto-save, and table customization.
...
Create or extend `backend/canvas-engine/src/test/java/org/chovy/canvas/controller/CanvasControllerEditorProductivityTest.java` with tests for authorization, tenant scoping, and the main success path named after `editor-productivity-beyond-baseline`. Use existing controller or service tests in `backend/canvas-engine/src/test/java/org/chovy/canvas` as style references.

- [ ] **Step 2: Run backend contract tests and confirm red state**

Run: `cd backend && mvn -pl canvas-engine test -Dtest=CanvasControllerEditorProductivityTest`
...
Create or extend `frontend/src/pages/canvas-editor/editorProductivity.test.tsx` with Vitest coverage for loading, empty, success, permission, and server-error states for the first UI slice.

- [ ] **Step 4: Run frontend workflow tests and confirm red state**

Run: `cd frontend && npm test -- editorProductivity.test.tsx`
...
**Files:**
- `backend/canvas-engine/src/main/java/org/chovy/canvas/web/CanvasController.java`
- Test: `backend/canvas-engine/src/test/java/org/chovy/canvas/controller/CanvasControllerEditorProductivityTest.java`
...
- `frontend/src/components/canvas/CanvasNode.tsx`
- `frontend/src/components/config-panel/index.tsx`
- Test: `frontend/src/pages/canvas-editor/editorProductivity.test.tsx`
...
Run: `git add backend/canvas-engine/src frontend/src docs/product-evolution/specs docs/product-evolution/plans && git commit -m "feat: implement editor-productivity-beyond-baseline slice"`
