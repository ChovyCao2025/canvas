...
**Goal:** Prevent white screens, dead routes, silent request failures, and baseline accessibility failures in the main app shell.
...
**Files:**
- Create or modify: `backend/canvas-engine/src/test/java/org/chovy/canvas/common/ErrorCodeTest.java`
- Create or modify: `frontend/src/components/errors/AppErrorBoundary.test.tsx`
- Read: `docs/product-evolution/specs/p0-002-frontend-resilience-and-a11y.md`

- [ ] **Step 1: Write backend contract tests**

Create or extend `backend/canvas-engine/src/test/java/org/chovy/canvas/common/ErrorCodeTest.java` with tests for authorization, tenant scoping, and the main success path named after `frontend-resilience-and-a11y`. Use existing controller or service tests in `backend/canvas-engine/src/test/java/org/chovy/canvas` as style references.

- [ ] **Step 2: Run backend contract tests and confirm red state**

Run: `cd backend && mvn -pl canvas-engine test -Dtest=ErrorCodeTest`
...
Create or extend `frontend/src/components/errors/AppErrorBoundary.test.tsx` with Vitest coverage for loading, empty, success, permission, and server-error states for the first UI slice.

- [ ] **Step 4: Run frontend workflow tests and confirm red state**

Run: `cd frontend && npm test -- AppErrorBoundary.test.tsx`
...
**Files:**
- `backend/canvas-engine/src/main/java/org/chovy/canvas/common/ErrorCode.java`
- Test: `backend/canvas-engine/src/test/java/org/chovy/canvas/common/ErrorCodeTest.java`
...
Run: `git add backend/canvas-engine/src frontend/src docs/product-evolution/specs docs/product-evolution/plans && git commit -m "feat: implement frontend-resilience-and-a11y slice"`
