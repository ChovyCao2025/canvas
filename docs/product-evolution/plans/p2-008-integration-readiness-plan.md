...
**Goal:** Package inbound webhooks, outbound webhooks, API keys, SSO/OIDC decision, data source management, and data sync into an integration foundation.
...
**Data And Config**
- `backend/canvas-engine/src/main/resources/db/migration/V102__integration_readiness.sql`
...
**Files:**
- Create or modify: `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/integration/IntegrationServiceTest.java`
- Create or modify: `frontend/src/pages/integrations/integrationReadiness.test.tsx`
- Read: `docs/product-evolution/specs/p2-008-integration-readiness.md`

- [ ] **Step 1: Write backend contract tests**

Create `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/integration/IntegrationServiceTest.java` with tests for authorization, tenant scoping, and the main success path named after `integration-readiness`. Use existing controller or service tests in `backend/canvas-engine/src/test/java/org/chovy/canvas` as style references.

- [ ] **Step 2: Run backend contract tests and confirm red state**

Run: `cd backend && mvn -pl canvas-engine test -Dtest=IntegrationServiceTest`
...
Create `frontend/src/pages/integrations/integrationReadiness.test.tsx` with Vitest coverage for loading, empty, success, permission, and server-error states for the first UI slice.

- [ ] **Step 4: Run frontend workflow tests and confirm red state**

Run: `cd frontend && npm test -- integrationReadiness.test.tsx`
...
- `backend/canvas-engine/src/main/java/org/chovy/canvas/domain/integration/IntegrationService.java`
- `backend/canvas-engine/src/main/resources/db/migration/V102__integration_readiness.sql`
- Test: `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/integration/IntegrationServiceTest.java`
...
- `frontend/src/pages/integrations/index.tsx`
- `frontend/src/services/integrationApi.ts`
- Test: `frontend/src/pages/integrations/integrationReadiness.test.tsx`
...
Run: `git add backend/canvas-engine/src frontend/src docs/product-evolution/specs docs/product-evolution/plans && git commit -m "feat: implement integration-readiness slice"`
