...
**Goal:** Explore smart send time, smart throttling, channel routing, marketing calendar, channel cost tracking, and WeCom-first expansion.
...
**Data And Config**
- `backend/canvas-engine/src/main/resources/db/migration/V105__channel_intelligence_scheduling.sql`
...
**Files:**
- Create or modify: `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/channel/ChannelIntelligenceServiceTest.java`
- Create or modify: `frontend/src/pages/channel-intelligence/channelIntelligence.test.tsx`
- Read: `docs/product-evolution/specs/p2-012-channel-intelligence-and-scheduling.md`

- [ ] **Step 1: Write backend contract tests**

Create `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/channel/ChannelIntelligenceServiceTest.java` with tests for authorization, tenant scoping, and the main success path named after `channel-intelligence-and-scheduling`. Use existing controller or service tests in `backend/canvas-engine/src/test/java/org/chovy/canvas` as style references.

- [ ] **Step 2: Run backend contract tests and confirm red state**

Run: `cd backend && mvn -pl canvas-engine test -Dtest=ChannelIntelligenceServiceTest`
...
Create `frontend/src/pages/channel-intelligence/channelIntelligence.test.tsx` with Vitest coverage for loading, empty, success, permission, and server-error states for the first UI slice.

- [ ] **Step 4: Run frontend workflow tests and confirm red state**

Run: `cd frontend && npm test -- channelIntelligence.test.tsx`
...
- `backend/canvas-engine/src/main/java/org/chovy/canvas/engine/handlers/PriorityHandler.java`
- `backend/canvas-engine/src/main/resources/db/migration/V105__channel_intelligence_scheduling.sql`
- Test: `backend/canvas-engine/src/test/java/org/chovy/canvas/domain/channel/ChannelIntelligenceServiceTest.java`
...
- `frontend/src/pages/channel-intelligence/index.tsx`
- `frontend/src/services/channelIntelligenceApi.ts`
- Test: `frontend/src/pages/channel-intelligence/channelIntelligence.test.tsx`
...
Run: `git add backend/canvas-engine/src frontend/src docs/product-evolution/specs docs/product-evolution/plans && git commit -m "feat: implement channel-intelligence-and-scheduling slice"`
