...
- Quick BI 产品页（2026-06-09 抽取）：`https://www.aliyun.com/product/quick-bi`，官方定位为大模型驱动数据分析产品，强调小Q报告/问数/解读/洞察/搭建五类 Agent、拖拽式仪表板/电子表格/数据大屏、多数据源连接、开放集成和安全协作。
- Quick BI 概述：`https://help.aliyun.com/zh/quick-bi/product-overview/introduction-to-quick-bi-1`
- Quick BI 数据源与支持的功能项（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/features-supported-by-different-data-sources`，官方说明所有数据源支持主链路数据分析，但初始化 SQL、上传文件、自定义周、HINT、Quick 引擎抽取加速、数据准备、数据填报、系统内置函数、趋势/多维分析等高阶功能按数据源类型差异支持。
- Quick BI 数据源 FAQ（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/faq-about-data-sources`，官方说明跨源关联需开启 Quick 引擎抽取加速，且探索空间数据表暂不支持关联其他数据源。
- 数据源支持概览（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/overview-of-data-sources-supported-by-quick-bi`，官方将数据源归为数据库、文件和应用/API 数据源，文件源支持 CSV/Excel，API 数据源通过外部接口获取业务数据且仅专业版支持。
- API 数据源创建（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/create-an-api-data-source`，官方流程为建立 API 连接、解析请求结果、数据同步设置；API 数据源支持 GET/POST、抽取/直连、参数/Header/Body/认证、JSON 结果解析、抽取更新周期和分页变量，直连结果量受 10MB、100 列、1000 行约束，且 API 数据源暂不支持跨源关联。
- 文件数据源创建（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/add-a-file-to-a-data-source/`，官方支持 Excel/CSV 上传、字段类型识别和调整、Sheet 选择、追加/替换/删除文件；推荐 CSV 使用 UTF-8，单文件建议不超过 50 MB，单 Sheet/CSV 列数不超过 100。
- 探索空间（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/explore-space`，官方说明探索空间支持上传 CSV/XLS/XLSX 作为独立数据源，适合临时数据和小型项目；高级版/专业版探索空间与抽取加速共用存储容量，基础容量分别为 2G/10G，组织管理员在 Quick 引擎内查看容量；探索空间不支持跨源关联，API 抽取、钉钉表格、飞书多维表格等抽取后也存储在探索空间容量内。
- 宜搭数据源（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/use-yida-as-a-data-source`，官方说明宜搭数据源仅专业版支持，需专属宜搭、应用专属存储和物理列模式，并通过连接测试后接入。
- 钉钉表格数据源（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/creat-a-dingding-spreadsheet-data-source`，官方说明钉钉表格数据源仅专业版支持，单表格页不超过 100 列和 10000 行、Sheet 不超过 5 个，不继承公式，且不支持监控告警和自助取数。
...
- 数据门户功能概述（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/bi-portals/`，官方定义为通过菜单形式组织仪表板、电子表格和外部链接的数据产品。
- PC 数据门户配置（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/create-a-pc-bi-portal-2`，官方能力包括顶部/左侧/双导航、LOGO、主副标题、页脚、面包屑、菜单缓存、菜单搜索、全屏、菜单主页、菜单图标和展示样式。
- 数据大屏配置（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/overview-of-the-data-visualization-ui` 和 `https://help.aliyun.com/zh/quick-bi/user-guide/getting-started-with-data-visualization`，官方资料显示大屏制作包含画布布局、组件配置和发布前配置检查链路。
- 电子表格配置（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/configure-a-workbook` 和 `https://help.aliyun.com/zh/quick-bi/user-guide/spreadsheet-style-configuration`，官方资料显示电子表格支持单元格区域、样式配置、条件格式、汇总小计和透视分析类配置。
- Ticket 嵌入安全增强（2026-06-09 抽取）：`https://help.aliyun.com/zh/quick-bi/user-guide/security-enhancement-solution-for-permissions-on-reports-and-parameter-passing-of-report-embedding`，官方增强方案支持仪表板、电子表格、数据大屏、自助取数、即席分析、数据填报嵌入，支持绑定用户、访问次数、有效期、水印、全局参数、区块嵌入和跳转链路。
...
- 自助取数下载概览（2026-06-09 抽取，2026-06-09 复核）：`https://help.aliyun.com/zh/quick-bi/user-guide/overview`，官方说明自助取数支持 Excel/CSV 下载，但 API 数据源、跨源数据集和探索空间上传数据源不支持自助取数；离线下载限制在 100 万行以内、文件 1GB 以内，文件有效期为 7 天。
- 组织级导出控制（2026-06-09 复核）：`https://help.aliyun.com/zh/quick-bi/user-guide/configure-the-export-feature`，官方导出控制覆盖组织级允许导出/公开链接导出开关、基础设置、普通导出设置和自助取数设置；自助取数可控制生成文件下载次数，普通导出适合 1 万行以内，超过 1 万行的交叉表可转自助取数。
...
- 指标监控告警（2026-06-09 复核）：`https://help.aliyun.com/zh/quick-bi/user-guide/configure-alert-rules`，官方监控告警支持仪表板和电子表格入口，监控指标可按维度拆分，检测时间支持每小时/每天/每周/每月；告警规则可基于度量值或同环比变化率，包含环比上小时、同比前一天、日环比、周环比、月环比、日_周同比和日_月同比；也支持波动分析、推送消息、历史数据保留和连续异常停止。
- 分析预警（2026-06-09 复核）：`https://help.aliyun.com/zh/quick-bi/user-guide/metric-analysis`，官方分析预警覆盖趋势线、预测、异常检测、波动原因和聚类；异常检测用于识别时间序列中偏离正常范围的点，波动原因分析要求时间维度和求和/计数类度量并支持多维归因。
...
能力覆盖率以 `docs/superpowers/evidence/quickbi-capability-benchmark.json` 为当前审计证据包，并由 `tools/strategy/quickbi-benchmark-audit.mjs` 在 `scripts/verify-quickbi-focus.sh` 中执行门禁校验。2026-06-09 的审计口径覆盖 30 个 Quick BI 对标能力项，按权重计算 Canvas BI 当前覆盖率为 95.5%，超过 90% 目标阈值；该结果证明能力覆盖目标已达到量化门槛，但仍保留 `partial`/`planned` 项，不替代完整测试、运行时验收和合并前工作树审计。

### 1.1 Quick BI 能力映射

| Quick BI 官方能力 | Canvas BI 对应设计 | 第一阶段落点 |
| --- | --- | --- |
| 数据源、数据集、字段建模 | `bi_data_source_ref`、连接器能力注册、凭证加密、连接测试、schema 同步、查询/抽取/缓存模式、`bi_dataset`、`bi_dataset_field`、`bi_metric` 和内置数据集注册表 | 已有 `canvas_daily_stats` 语义模型、元数据表、BI 数据源连接器目录、租户内已配置数据源可见性、BI 数据源创建/编辑接入 API 与工作台入口、连接器类型和默认连接模式持久化、连接器支持模式的创建期约束、API HTTP/JSON 抽取接入配置持久化、CSV/Excel 文件上传、schema 同步、数据集草稿生成和 EXTRACT 物化刷新闭环、运行时预览和工作台数据预览入口、凭证加密和轮换、真实连接测试、schema 预览、持久化 schema 同步快照、API schema sync 多组 runtime 变量、基于成功 schema 快照生成单表和多行多表复合 Join 条件数据集草稿、API 响应 schema 到 EXTRACT 数据集草稿、SQL 模板参数后端绑定、前端 SQL 模板、参数、字段/指标精细编辑、样例预览和血缘影响分析入口、数据集抽取加速策略、手动刷新运行记录、API 抽取调度运行与逐策略观测、API 和文件数据集 EXTRACT 刷新写入 `bi_extract` 物化表、数据源接入后的 QuickBI-style 下一步动作/限制提示、API/APP/探索空间文件容量池策略说明，以及数据源 `USE`/`EDIT` 资源权限 foundation；更深的数据源能力差异约束仍需补齐 |
| 仪表板、查询控件、组件拖拽 | BI 工作台左侧资源区、中间 20 栅格画布、右侧属性面板、顶部工具栏、运行态参数和级联查询控件 | 已有 `canvas-effect` 预置看板和 QuickBI-like 设计器；URL 参数、全局参数 key/alias、控件默认值、记住查询条件、控件作用范围、条件级联候选查询和 embed 参数 claim 绑定已进入运行态查询链路；运行态编辑器已展示每个控件当前值来源（URL 覆盖、记住条件、默认值、已清除）和锁定状态，并按控件类型提供日期范围双输入、枚举多选候选按钮和文本兜底输入；已保存图表资产支持基础编辑器、字段表单和字段点击/拖放槽生成结构化查询、复制草稿和引用影响摘要 |
| 钻取、联动、跳转 | `BiDashboardInteraction` 保存组件关系和跳转目标 | 已在预置看板中表达联动、钻取、超链接 |
| 数据门户、自助取数、电子表格、大屏 | 门户菜单、导出任务、表格式分析和大屏资源类型 | 门户资源 lifecycle、自助取数/导出 foundation、电子表格和大屏资源 lifecycle、运行视图、工作台可视化编辑控件已完成；大屏布局已支持选中组件的方向移动、宽高步进缩放、多组件对齐、邻近组件参考线吸附和移动端单列/紧凑双列布局变体；电子表格运行态已支持单元格引用、基础算术公式、`SUM`/`AVERAGE`/`MIN`/`MAX`/`COUNT` 区间和多参数聚合求值、除零错误标记和移动端列数适配，工作台支持单元格范围批量填充和单元格粗体/背景色/文字色编辑；大屏/电子表格工作台已新增视觉诊断摘要，发布前提示大屏组件重叠、栅格越界、移动端布局覆盖、电子表格公式错误、样式、条件格式和透视表覆盖 |
...
| 工作空间角色、行列权限 | 工作空间成员、资源权限、数据源权限、行权限、列权限、权限申请和审计 | 查询链路已接入资源/行/列权限、工作空间成员角色解析、脱敏和拒绝审计；资源/行/列权限已有管理 API 和工作台基础 UI；数据源 `USE` 权限已用于从数据源创建数据集，数据源 `USE`/`EDIT` 授权可通过权限 API 和工作台授予；权限申请已支持申请、列表、审批和审批通过后自动授予申请人资源权限；嵌入组件查询已复用同一查询执行权限/行列权限链路；订阅手动运行、调度运行、告警、投递重试和附件下载已统一复用 `ACTION_SUBSCRIBE` 二次鉴权 |
...
- API/APP 数据源会按认证方式约束凭证：`NONE` 不要求用户名/密码并保存为空凭证，`BASIC`/`BEARER`/`API_KEY` 仍要求凭证；JDBC 数据源会按连接器能力目录校验 driver class allowlist，避免 PostgreSQL、Oracle、SQL Server 等连接器被错误 driver 创建。
...
- 建模方式已支持可视化表/视图建模、schema 快照驱动的多行多表关联建模，并新增 SQL 数据集草稿入口；SQL 草稿由服务端限制为单条只读 `SELECT`、拒绝危险 SQL token/注释/多语句、要求包含租户列和明确 `FROM` 源表，并支持 `{{parameter_key}}` 模板参数定义、默认值、必填、枚举允许值和 JDBC `?` 占位符绑定；前端工作台可输入 SQL 模板、自动解析模板参数、配置参数类型/必填/默认值/允许值，精细编辑 SQL 字段和指标，并通过既有草稿保存接口提交；SQL 样例预览和血缘影响分析已复用同一草稿资源模型，预览结果会从样例列/行派生字段级样本剖析，展示每列角色、类型、填充率、唯一值数和示例值，并把影响资产、血缘来源、运行参数、引用字段/指标、治理门禁和风险提示汇总成结构化影响分析摘要；SQL 面板新增发布诊断摘要，会在保存/预览前后统一提示字段指标完整度、必填参数默认值、样例执行、血缘来源、治理门禁、发布审批和风险告警。查询编译会按 `model.sqlParameterOrder` 在租户过滤和运行态筛选前绑定 `BiQueryRequest.sqlParameters`，缺少必填参数或传入非法枚举值时阻断。SQL 数据集发布强制走发布审批且管理员不能绕过。后续仍需补全更完整 SQL lint。
...
- Quick BI 官方物理模型资料显示：数据建模采用关系模型画布与物理模型画布双层结构；物理模型支持 LEFT/RIGHT/INNER/FULL JOIN、多个关联键、`=`/`<>`/`>`/`>=`/`<`/`<=` 关联符号、关联前过滤和自定义关联字段；关联表最多支持 5 层，MySQL 数据源暂不支持 FULL JOIN，跨源关联和合并需要 Quick 引擎抽取加速，API 数据源仅抽取模式支持关联和合并。Canvas BI 的 schema 快照多表建模已支持可点击 graph-canvas 关系边、拖拽节点、多 Join 类型、多字段 Join 条件、非默认操作符、OR 连接、括号分组、方向交换、同名字段批量添加和最终命令持久化；多表关系诊断会按 Quick BI 对标口径展示建模表、关联层级、Join 类型、关联条件复杂度和关系覆盖，并对超过 5 层、FULL JOIN、OR/分组条件、未覆盖表等风险给出可发布/需复核/需补齐状态。
...
- 前端工作台“数据集资产”表支持选择数据集资产，并在“数据集发布历史”表格查看发布版本、发布人、发布时间和恢复入口；数据集基础编辑器可选择多个字段批量写入字段文件夹 `folderKey`，以文件夹分组方式展示字段大纲，可选中单个字段编辑展示名、角色、语义类型、默认聚合、格式和可见性，也可对选中字段批量应用角色、格式和可见性配置；字段大纲支持用选择控件或原生拖放把字段移动到目标字段前后，重写 `sortOrder` 并可同步移动到目标文件夹；数据集可复制为 `*_copy` 草稿后复用 `POST /canvas/bi/datasets/resources/{datasetKey}/draft` 保存；数据集资源治理支持输入多个 dataset key 后批量移动到目标文件夹或批量转让 owner，逐个复用统一 `POST /canvas/bi/resources/move` 和 `POST /canvas/bi/resources/transfer` 权限链路。数据源 schema 快照表支持从单表结构直接生成并选中数据集草稿，API 快照会把当前 API 多组预览变量写入数据集创建命令并在无响应租户列时仅设置统一 `tenant_id` 租户列配置；多表建模面板支持选择多张表、主表、添加多条关联行、配置每条 Join 的左右表和可重复字段条件，并在关系画布中显示可点击 Join 边、Join 类型、字段摘要/条件数和可拖拽表节点，分析师可选中关系边并从画布上下文设置 Join 左右端点表、Join 类型、查看、编辑字段和条件操作符、添加、移除多字段 Join 条件，并一键添加全部同名字段条件，也可直接交换关系方向以互换左右表、左右字段和复合条件字段对，生成星型或链式 `conditions[]` 与 `GRAPH_CANVAS` 节点布局的多表 SQL 数据集草稿，服务端会把复合条件、graph 节点和 Join 派生边持久化到模型并生成 `AND` 连接的 SQL `ON` 子句；关系画布新增关系诊断摘要，展示建模表数量/主表、关联层级、Join 类型风险、复合条件/OR/括号复杂度和关系覆盖状态；数据源接入区域新增 SQL 数据集面板，支持输入 dataset key/name/tenant column/SQL 模板、解析 `{{parameter_key}}`、配置参数类型/必填/默认值/允许值，精细编辑字段 key、展示名、表达式、角色、类型、可见性、语义类型、默认聚合、格式、单位、敏感级别，以及指标 key、展示名、表达式、聚合、类型、允许维度、单位、格式、负责人和描述，并保存为 SQL 数据集草稿或发起样例预览与血缘影响分析；同一区域会汇总发布诊断行，把字段/指标、运行参数、样例预览、血缘审批和风险提示按可发布/需复核/需补齐呈现。
- `BiDatasetSpecResolver` 将持久化数据集转换为查询语义模型；查询编译、查询执行和图表保存优先使用 resolver，找不到时回退内置营销数据集。
- 前端工作台底部展示“数据集资产”，并保留营销预置数据集展示。

权限申请和权限管理 UI 深化仍需继续收口。资源移动/转让/收藏/评论/编辑锁 foundation、保存与版本恢复写入链路编辑锁强制校验、发布审批、保存/发布权限检查、SQL 参数后端绑定、前端 SQL 参数草稿配置、SQL 字段/指标精细编辑、SQL 样例预览/血缘影响分析和发布诊断、字段文件夹基础写入、字段大纲、字段详情、字段批量配置、字段文件夹拖拽式编排、数据集资源批量移动/批量转让、数据集复制草稿、schema 快照驱动多表复合 Join 建模与行列权限的查询期执行基础已接入；剩余 SQL 数据集风险主要集中在后端 SQL lint 完整度、更多方言函数白名单和生产级错误解释。
...
- 嵌入页复用工作台同一套运行态解析 helper，将已签名 ticket payload 中的 `filters` 与全局 `parameters` 合成为 dashboard runtime parameters，再用于组件查询和页面运行态标签展示，避免外部嵌入与认证工作台出现参数双轨；工作台交互面板会在生成嵌入 ticket 前预览同一请求即将固化的资源、scope、TTL、运行态过滤和全局参数，帮助分析师确认嵌入链接使用的上下文。
...
- `GET /canvas/bi/charts/resources/{chartKey}/impact` 基于服务端全量元数据返回该图表被仪表板 widget、门户菜单和订阅任务引用的影响范围。
...
- 前端工作台已接入图表基础编辑器：可编辑已保存图表的图表类型、数据集、维度字段、指标字段、基础筛选字段/操作符/值、基础排序字段/方向和取数 limit，支持从当前数据集字段点击或拖放到维度/指标槽生成查询字段，并把表单转换为结构化 `BiQueryRequest` 后复用 `POST /canvas/bi/charts/resources/{chartKey}/draft` 保存草稿；样式编辑已覆盖主题、密度、调色板、图例开关、数据标签开关、X/Y 轴标题、标签位置、数字格式和单规则条件格式（字段、操作符、阈值、颜色），交互编辑已覆盖钻取开关、联动目标和跳转模板，并随同图表草稿写入 `style` / `interaction` JSON；复制图表会生成 `*-copy` 草稿并保留查询、样式和交互配置；编辑器优先展示服务端全量元数据返回的仪表板 widget、门户菜单和订阅任务图表引用影响摘要，接口失败时回退到当前已加载资源的本地摘要。

后续仍需要补齐：按图表类型细分的高级样式面板、多规则条件格式、轴线/标签细粒度配置和交互规则编排器。图表资源可通过统一资源管理 band 进行移动/转让/收藏/评论/锁定，并通过统一 lifecycle guard 执行保存/发布权限、保存与版本恢复写入链路编辑锁与发布审批。
...
当前已落地大屏资源草稿/发布 lifecycle、运行态全屏栅格展示，以及工作台内对选中组件标题、资源绑定、x/y/w/h 坐标尺寸、方向移动、宽高步进缩放、多组件对齐、邻近组件参考线吸附和移动端单列/紧凑双列布局变体的可视化编辑与保存；工作台已接入大屏组件库基础模板，可添加指标卡、趋势折线、排行列表和文本面板组件并写入草稿 layout；视觉诊断摘要会统计组件数量、重叠数量、24 栅格越界数量和移动端布局覆盖状态，在发布前暴露阻断或复核风险。
...
当前已落地电子表格资源草稿/发布 lifecycle、运行态网格渲染，以及工作台内 sheet/cell 选择、单元格值/公式编辑、空值清除、范围批量填充、单元格粗体/背景色/文字色编辑和保存；运行态支持单元格引用、`+`/`-`/`*`/`/` 基础算术公式、括号优先级、`SUM(A1:B2)`、`AVERAGE(A1:B2)`、`MIN(A1:B2)`、`MAX(A1:B2)`、`COUNT(A1:B2)` 区间公式和多参数聚合求值，并对循环引用和除零展示错误标记，会渲染已保存的单元格样式，且可按 sheet 级 `mobileLayout` 限制移动端列数；工作台已支持基于源单元格区域生成交叉表透视配置与汇总结果单元格；视觉诊断摘要会统计单元格、公式、错误值、显式样式、条件格式和透视表数量，帮助分析师在保存或发布前复核复杂工作簿。
...
- `GET /canvas/bi/self-service/exports/{id}` 返回当前租户内单个导出任务的审计详情，包含任务状态、存储、下载、审批、重试元数据以及恢复后的原始结构化导出请求；对象级分片 ZIP 会解析 manifest 并返回 `storageLayout`、请求/生成行数、分片数、每片大小和 part storage key。
...
- 前端 BI 工作台展示字段拖拽取数构建器、自助取数预览、普通 CSV 导出、敏感导出申请、导出任务列表、审批状态、审批意见输入、批准/驳回操作、可取消任务入口、保留期、下载次数、过期状态、进度条、失败重试状态、手动重试结果、清理结果、下载入口和导出审计详情抽屉；分片导出审计详情会展示对象级布局、生成/请求行数、分片数量、每片大小和可读的 part 对象文件名；导出任务区新增硬化诊断摘要，按导出控制、分片存储、留存下载和重试恢复四个维度聚合任务数、审批中、过期、对象分片、百万行生成进度、7 天留存、下载审计、重试中和重试耗尽状态。
...
- 前端工作台已接入门户基础编辑器：可编辑顶部/左侧/双导航布局、默认主页、菜单搜索开关、全屏开关、移动端开关、LOGO、主标题、副标题、页脚、门户别名、菜单层级面包屑开关和菜单缓存 TTL；菜单配置支持修改标题、父级菜单和图标 key，可选择菜单执行上移/下移重排，也可按 before/after/inside 语义将菜单拖放到目标菜单前后或作为子菜单；保存门户草稿复用 `POST /canvas/bi/portals/resources/{portalKey}/draft`，有效编辑锁会随请求携带；编辑器内提供门户预览块，按当前草稿展示标题、副标题、菜单入口和页脚。
- `POST /canvas/bi/embed/resources/portal` 提供匿名、ticket 绑定的数据门户元数据读取接口，校验 ticket 的资源类型、资源 key、来源域名、访问次数和有效期后，使用 ticket 内的租户和用户名读取已发布门户。
- `/bi/embed/PORTAL/{portalKey}?ticket=...` 已支持门户嵌入渲染：验证门户 ticket 后加载已发布门户菜单，展示门户标题、副标题、菜单资源入口和页脚；菜单入口可在嵌入页内切换当前打开资源。`DASHBOARD` 菜单会使用同一个门户 ticket 读取菜单声明的仪表板资源并执行组件查询，后端只允许访问已发布门户可见菜单中声明的 dashboard resourceKey，非仪表板菜单仍只展示资源摘要或外链入口。

后续仍需要补齐：更完整的门户主题视觉预览，以及电子表格、大屏、自助取数等非仪表板菜单资源内容的服务端授权装载链路。
...
- 前端工作台新增“订阅推送”和“指标告警”区域，支持查看已有任务、创建当前看板日报订阅、为当前选中指标创建阈值告警或基线异常告警、手动执行/检测、触发一次调度检查、重试失败/待配置投递、查看投递记录与审计汇总、查看重试次数/下次重试/耗尽状态、下载投递附件、查看附件过期/下载次数并手动清理过期附件；指标告警区新增异常诊断摘要，按异常覆盖、周期边界、节假日映射和样本静默四个维度展示同环比异常数量、停用规则、自然边界、calendar tolerance window、holiday comparison date/name、最小样本和静默配置。
- 告警异常模型已支持点异常、移动平均和 `PERIOD_OVER_PERIOD` 同比/环比模型，覆盖日环比、周同比、月环比、年同比；周期模型支持可配置 calendar tolerance window、自然周期边界对齐，以及节假日 comparison date/name 映射，检测日志 payload 会写入 period、target window、naturalBoundary、holidayAdjusted、holidayComparisonDate、baseline sample 和 delta 细节；工作台异常诊断会把这些能力以 QuickBI-style 指标监控口径展示给运营和空间管理员复核。
...
- `POST /canvas/bi/embed/resources/dashboard` 匿名加载签名 ticket 授权的仪表板资源元数据，先校验 ticket 资源范围，再按来源白名单扣减访问次数/频率窗口，并使用 ticket 的 `tenantId` 读取持久化仪表板资源或预置兜底；当 ticket 绑定资源为 `PORTAL` 时，仅允许读取该门户已发布可见菜单中声明的 `DASHBOARD` resourceKey。
- `POST /canvas/bi/embed/resources/dashboard/runtime-state` 匿名加载签名 ticket 授权的仪表板运行态参数，先校验 ticket 资源范围，再按来源白名单扣减访问次数/频率窗口，并使用 ticket 的 `tenantId`、`username` 和请求的 dashboard `resourceKey` 读取当前用户记住的运行态状态；当 ticket 绑定资源为 `PORTAL` 时，仅允许读取该门户已发布可见菜单中声明的 `DASHBOARD` resourceKey。
- `POST /canvas/bi/embed/query/execute` 匿名执行签名 ticket 授权的仪表板组件查询，先校验 ticket 资源范围和 `query.dashboardKey`，再扣减访问次数/频率窗口，并以 ticket 的 `tenantId`/`username` 构造 `BiQueryContext` 复用查询执行、资源权限、行权限和列权限链路；门户 ticket 只能执行其已发布可见菜单中声明的 `DASHBOARD` 查询，且 request resourceKey 必须与 `query.dashboardKey` 一致。
...
- 匿名嵌入页 `/bi/embed/:resourceType/:resourceKey` 已具备基础路由、ticket 校验、资源匹配校验、票据化持久化看板资源加载、票据化运行态读取、逐组件签名查询渲染，以及门户菜单内仪表板资源的真实数据渲染；直接 dashboard ticket 和门户菜单 dashboard 复用同一套嵌入运行态合成与查询执行流程。
- 当前 ticket 已支持 `bi_embed_token` token hash 持久化、nonce/source 绑定、数据库原子访问次数与频率窗口扣减、访问审计、租户级过期回收、访问次数/频率窗口限制、embed 参数 claim 绑定、票据化仪表板资源元数据读取、票据化运行态读取、门户菜单内仪表板资源读取、真实数据查询渲染和嵌入态查询权限链路复用；嵌入页运行态参数合成遵循 ticket filters/parameters 优先、记住查询条件次之、控件默认值兜底的 QuickBI-like 优先级。
...
- 前端 BI 工作台已展示资源授权、行权限、列权限三类规则，资源授权目标与共享资源操作目标分离；权限治理可选择当前仪表板、图表、数据集、数据源、门户、大屏或电子表格授予 `USE`/`EDIT`，可按主体类型、主体 ID、动作和允许/拒绝效果保存资源授权规则，可按规则 key、主体类型、主体 ID、过滤 JSON 和启用状态保存数据集行权限规则，可按字段、主体类型、主体 ID、策略、mask JSON 和启用状态保存数据集列权限规则，可删除资源授权、行权限和列权限规则，可按当前 BI 资源提交 `USE`/`EDIT`/`EXPORT`/`PUBLISH` 权限申请并在同一 workbench 审批待处理申请。
...
- 订阅手动运行、订阅调度运行、告警运行、失败/待配置投递重试和投递附件下载已复用 `BiPermissionService.ACTION_SUBSCRIBE`，真实投递路径会按绑定的工作空间、资源类型和资源 ID 进行二次鉴权。
...
- 上传文件和 API 数据源闭环：`POST /canvas/bi/datasources/file-upload` 支持租户隔离的 CSV/XLS/XLSX 上传接入，`POST /canvas/bi/datasources/file-upload/materialize` 支持上传后立即 schema sync、生成数据集草稿、启用 EXTRACT 策略并刷新物化表；无租户列文件会在建模与抽取时保留统一 `tenant_id` 配置并在物化写入时注入当前租户，前端工作台会在上传成功后刷新数据源、schema、数据集资产和加速策略状态，并按 API、APP、CSV/Excel、JDBC 数据源展示接入后的就绪状态、下一步动作、QuickBI-style 限制提示、容量池、预算/约束、自助取数适用性和治理动作，避免把 API/探索空间文件错误引导进自助取数或跨源分析。
...
- 持久化门户资源：支持门户资源列表、详情、保存草稿、发布、归档、发布快照历史、历史版本恢复和按当前用户/角色过滤的 runtime 查看态；前端工作台支持门户基础编辑器，可保存导航布局、默认主页、菜单搜索、全屏、移动端、LOGO、标题、副标题、页脚、别名、面包屑、菜单缓存 TTL、菜单标题、父级菜单、图标 key、菜单上下移动排序和 before/after/inside 菜单树拖放编排。
...
- 嵌入 ticket：`POST /canvas/bi/embed-tickets` 签发短期 HMAC ticket；外部 ticket 必须携带允许域名并把规范化 host/port 写入签名 payload；配置持久化 mapper 后会把 token hash、resource key、nonce、过期时间、签名 scope、全局参数、最大访问次数和频率窗口元数据落入 `bi_embed_token`；`POST /canvas/bi/embed-tickets/verify` 匿名 HTTP 校验会按 `Origin`/`Referer` 执行来源白名单并通过数据库原子更新完成访问次数与频率窗口扣减，成功和 replay/限流拒绝会写入 `bi_audit_log`；`POST /canvas/bi/embed/resources/dashboard` 匿名读取 ticket 授权的 dashboard resource 元数据，并用 ticket 租户读取持久化资源，门户 ticket 仅能读取菜单内 dashboard；`POST /canvas/bi/embed/resources/dashboard/runtime-state` 匿名读取 ticket 授权的 dashboard runtime state，并用 ticket 租户、用户和请求 dashboard key 读取记住查询条件，门户 ticket 同样限定在菜单内 dashboard；`POST /canvas/bi/embed/query/execute` 匿名执行签名 ticket 授权的 dashboard widget query，并用 ticket 上的租户和用户进入同一查询权限/行列权限链路；`POST /canvas/bi/embed-tickets/cleanup` 会按当前租户批量回收过期 token；未配置 mapper 时仍保留进程内 nonce fallback；前端设计器可按 widget 数量加 verify、资源元数据加载和运行态读取生成可完成一轮渲染的访问次数上限。
- 前端工作台：QuickBI-like 设计器布局、持久化/预置看板画布、门户查看态、订阅告警、资源位置移动、资源负责人转让、资源收藏、资源/组件评论、资源编辑锁、发布审批请求/审核、权限申请/审核、持久化门户资产列表、持久化数据集资产列表、持久化图表资产列表、数据集基础编辑器、图表基础编辑器、看板复制/归档/导入/导出、资源包 JSON 文件下载/上传、看板/图表/数据集/门户发布历史和版本恢复、数据/样式/交互面板、组件库、查询控件、数据集清单；数据集基础编辑器可批量写入字段文件夹并复制数据集草稿，图表基础编辑器可保存图表类型、数据集、维度、指标、基础筛选、基础排序和 limit，支持字段点击/拖放到维度和指标槽生成查询字段，可复制草稿并优先展示服务端全量引用影响摘要；看板保存草稿以及看板/图表/数据集/门户版本恢复会在当前资源锁有效时携带锁 token；看板 widget 会按维度/指标自动执行查询，失败时降级为预置预览；工作台展示数据源健康、数据源连接器目录、连接器容量策略、租户已接入数据源、数据源接入后续动作和限制提示、数据源连接测试结果、API 数据源真实行预览、CSV/Excel 文件选择上传与一键物化、schema 预览、schema 同步快照、从 schema 快照生成单表/多表复合 Join 数据集草稿、最近健康历史、健康 SLO 摘要、最近查询历史、查询详情抽屉、当前查询治理策略、最近治理策略审计、最近权限变更审计、当前查询缓存策略、当前查询缓存 provider/容量/TTL/命中率/写入驱逐统计、Quick 引擎容量水位/分类/Top 资源/用户排行/容量告警策略/租户容量池策略/并发队列观测、数据集加速策略和 widget 的实时/缓存状态，可更新默认 timeout/quota、默认缓存开关/TTL、当前仪表板/当前数据集缓存覆盖策略的模式/TTL/开关、Quick 引擎容量上限/预警阈值/严重阈值/通知渠道/接收人、Quick 引擎 pool key/并发上限/队列上限/排队超时/容量池权重、数据集加速模式/刷新策略，可按当前数据集或全量发起清缓存并手动抽取刷新，并可对有 SQL hash 的查询记录发起取消请求；设计器资源区支持字段/图表/控件搜索，组件卡片支持查看 SQL、复制、删除、布局微调、Ctrl/Shift/Meta 多选、左/中/右/顶/垂直居中/底对齐、桌面/平板/手机预览模式、标题区拖拽移动、吸附到邻近边线或中心线、右下角拖拽缩放、真实 CSS 栅格定位、碰撞避让和顶部撤销/重做，右侧数据面板展示查询结果、SQL hash、明细预览、后端编译出的参数化 SQL 和执行计划诊断；URL 中以 filter key、字段名、全局参数 key 或全局参数 alias 传入的查询控件参数会绑定到 widget 执行、SQL 编译、执行计划、自助取数查询和 embed ticket claims，并按控件类型规范化为日期区间、枚举多选、数值区间和单值控件；运行态参数按 URL 参数、当前用户记住查询条件、控件默认值的优先级合成，`LAST_7_DAYS`/`LAST_30_DAYS` 等动态默认值会解析为具体日期区间，控件作用范围会限制只过滤目标 widget；查询控件候选值通过 `buildDashboardControlOptionQuery(...)` 按父控件级联、字段映射和当前运行态参数执行查询，并在交互面板展示当前候选值；画布工具栏和右侧交互面板可通过类型化编辑器直接编辑、同步到 URL、持久化并解释当前运行态参数来源。
- 前端嵌入页：`/bi/embed/:resourceType/:resourceKey` 在无登录态下校验 ticket，通过 `POST /canvas/bi/embed/resources/dashboard` 加载签名 ticket 授权的持久化仪表板资源，通过 `POST /canvas/bi/embed/resources/dashboard/runtime-state` 加载签名 ticket 授权用户的记住查询条件，并按 ticket filters/parameters、记住查询条件、控件默认值的优先级还原运行态参数，再逐 widget 调用签名嵌入查询接口渲染 KPI、折线、柱图和表格结果；直接 dashboard ticket 和门户菜单 dashboard 共享这条运行态加载和查询执行路径，没有查询结果时显示空状态，不再用静态假数据填充。
- Canvas 入口：画布统计页和画布列表可进入 `/bi?dashboard=canvas-effect&canvasId={id}`。

仍未完成的生产级能力：

- 数据源中心闭环：连接器目录、连接器类型、容量分类和默认连接模式持久化、支持模式创建期约束、多步骤接入向导、API/APP HTTP/JSON 抽取接入配置持久化、API/APP 数据源真实预览、API 数据集 EXTRACT 物化执行路径、API 抽取刷新调度 UI/观测、CSV/Excel 文件上传、schema 同步、数据集草稿生成和 EXTRACT 刷新闭环、已配置数据源脱敏可见性、数据源接入后续动作和自助取数/跨源限制提示、API/APP/探索空间文件容量池策略提示、高阶功能支持矩阵、真实 JDBC 连接测试、表/字段 schema 预览、schema 同步持久化快照、单表数据集草稿生成、多行多表复合 Join 条件建模草稿生成、SQL 模板参数后端绑定、前端 SQL 模板与参数配置草稿入口、SQL 字段/指标精细编辑、SQL 样例预览和血缘影响分析、凭证轮换、数据集抽取加速基础运行态、数据源 `USE`/`EDIT` 权限 foundation、API/APP 认证方式凭证约束、JDBC driver allowlist 和健康 SLO 风险分级/治理建议已完成；更深连接健康治理、VPC/白名单排障向导和更完整的官方数据源功能项逐连接器落库仍需补齐。
- 动态报表运行态：URL filter key/字段名/全局参数 key/全局参数 alias 到 widget 查询、SQL 编译、执行计划、自助取数查询和 embed ticket claims 的绑定已完成；当前用户看板运行态状态保存/读取、记住查询条件、控件默认值解析、筛选影响范围、查询控件候选值条件级联、日期范围/枚举多选/文本类型化运行态编辑、URL 规范化写回、运行态来源状态展示和嵌入态运行态复用已接入。
...
- 生产级自助取数硬化：流式分片写入、对象级分片保留、超大导出取消、下载限流、分片下载审计、跨 provider 根对象/分片对象恢复演练和前端导出硬化诊断摘要已完成。
- 电子表格和大屏资源 lifecycle、运行视图与工作台可视化编辑控件已完成，大屏组件库基础模板、选中组件方向移动、宽高步进缩放、多组件对齐、邻近组件参考线吸附和移动端单列/紧凑双列布局变体、发布前视觉诊断、电子表格单元格引用、基础算术公式、常见聚合区间和多参数公式求值、单元格粗体/背景色/文字色编辑、移动端列数适配、交叉表透视生成、单元格/公式/样式/条件格式/透视表覆盖诊断、节假日感知和自然周期边界对齐的同比/环比异常模型及工作台异常诊断已接入。
...
- SQL 数据集上线前必须提供样例预览、源表血缘和影响分析：预览应复用保存时的只读 SQL lint、参数模板绑定、租户字段校验和查询编译器，返回编译 SQL、样例列/行、数据源 ID、FROM/JOIN 源表、参数顺序、引用字段/指标、发布审批门禁、缓存和下游报表影响；前端基于样例列/行展示字段级样本剖析，包括每列角色、类型、填充率、唯一值数和示例值，帮助分析师在发布前判断字段/指标是否可用；同时展示影响资产、血缘来源、运行参数、引用字段/指标、治理门禁和风险提示，避免只用零散标签表达上线影响；执行失败时只返回诊断，不伪造样例行。
- 数据门户参考 `bi-portals`、`create-a-pc-bi-portal-2` 和 `bi-portal-menu-permissions`：门户不是单报表页面，而是菜单化的数据产品；需支持顶部导航、左导航、双导航、LOGO、主副标题、页脚、面包屑、菜单缓存、菜单搜索、默认主页、全屏、菜单图标/展示样式、空间资源/外部链接菜单和菜单级授权。
...
- 嵌入参考 `basic-scheme-for-report-embedding`、`security-enhancement-solution-for-permissions-on-reports-and-parameter-passing-of-report-embedding` 和 `CreateTicket`：ticket 默认短有效期、可限制使用次数、绑定用户或账号、绑定资源和筛选参数；增强嵌入需覆盖仪表板、电子表格、数据大屏、自助取数、即席分析和数据填报，支持全局参数、水印、区块嵌入和多级跳转链路。Canvas 的实现用 HMAC ticket 替代阿里云 OpenAPI，但安全语义保持一致。
