...
- 自助取数/导出 foundation：`/canvas/bi/self-service/preview`、`/canvas/bi/self-service/exports`、`/canvas/bi/self-service/exports/{id}/review`、`/canvas/bi/self-service/exports/{id}/download`、`/canvas/bi/self-service/exports/queue/run`、`/canvas/bi/self-service/exports/retry` 和 `/canvas/bi/self-service/exports/cleanup` 支持授权数据集预览、CSV/JSON/XLSX 异步导出任务、敏感/大批量导出审批、下载、下载审计、过期拒绝、失败重试和本地过期文件清理，前端工作台已提供预览、普通导出、敏感导出、任务列表、待审批通过/驳回、进度/重试状态、过期/下载状态、手动重试和清理入口。
...
- 生产级自助取数：对象存储、百万行分片、审计详情页、Excel 样式和 PDF 导出。
