...
4000 execution 并发可以作为 3000 达标后的上限扩展目标，但不能与 3000 目标混同。只有当 3000 压测显示 Redis、MySQL、RocketMQ、下游、worker、retry backlog 都仍有明确余量时，才进入 4000 readiness；否则应先解决 3000 暴露出的瓶颈。
...
4000 readiness 的建议预算只作为下一阶段起点，不能直接用于生产承诺：
- `LIGHT`: 800
- `STANDARD`: 2400
- `HEAVY`: 400
- `RETRY`: 400
...
进入 4000 前，必须先证明 3000 目标下 `LIGHT` / `STANDARD` 仍有 RT 余量，`HEAVY` / `RETRY` 不会造成主链路退化。
...
4000 readiness 要求从 lane metadata 进一步升级到 worker 隔离：
- `LIGHT` / `STANDARD` 必须有可独立保护的 worker pool。
- `HEAVY` 和 `RETRY` 不能共享主链路 worker 上限。
- 如果继续使用单 ring buffer，必须证明 worker 饥饿不会发生；否则升级为 per-lane ring buffer。
...
4000 readiness 要求 retry 具备自适应退避：
- 根据 retry backlog、下游超时率、DLQ 增长率动态降低 retry 速率。
- 当 `LIGHT` / `STANDARD` RT 退化时，自动压低 `RETRY` lane 预算或延长退避。
- retry 恢复速度服从主链路保护，不以最快清空 backlog 为目标。
...
4000 readiness 要求按 DAG 成本分级，而不是只按触发类型或节点类型分级。画布发布时应计算 execution cost profile：
- 节点数量。
- 外部调用数量。
- Groovy 节点数量。
- Audience / replay / batch 标记。
- 预估同步 Redis / DB 往返。

执行时以 cost profile 作为 lane 解析输入，防止复杂 DAG 误入 `STANDARD`。
...
4000 readiness 要求状态写入进一步异步化：
- execution 主状态保留同步写。
- trace、审计、统计、控制台展示类数据进入 buffer / batch writer。
- 弱在线写入不能和 execution 主状态竞争同一热表写路径。
- Redis 角色不再只是客户端抽象，应完成 execution-state、route-cache、bitmap / large-object 的物理分层或等价隔离。
...
4000 readiness 要求下游 bulkhead 成为硬门槛：
- 每个外部依赖独立连接池。
- 每个外部依赖独立限流和熔断。
- 慢下游只能消耗自己的预算，不能占用全局 execution slot 直到超时。
- 没有容量契约的下游调用必须降级或移入 `HEAVY` / 异步路径。
...
### Phase 5：4000 readiness 上限扩展

目标：
- 在 3000 正式验收通过后，验证 4000 execution 并发的架构余量。
- 将 lane metadata 隔离升级为 worker 隔离。
- 将弱在线状态和审计写入从主执行热路径中移出。
- 完成 Redis 物理分层或等价隔离。
- 将 retry 退避升级为自适应控制。
- 基于 DAG cost profile 做 lane 解析。

进入条件：
- 3000 并发完整观察窗口稳定。
- 3000 下 `LIGHT` / `STANDARD` p95 / p99 RT 仍有明确余量。
- 3000 下 Redis / MySQL / RocketMQ / 下游没有持续饱和。
- retry backlog 和 DLQ 增长可控。
- `HEAVY` 压力不会造成主链路 worker 饥饿。

通过条件：
- 4000 并发完整观察窗口稳定。
- `LIGHT` / `STANDARD` 不被 `HEAVY` / `RETRY` 拖垮。
- 弱在线写入和审计写入不造成 MySQL 热表争用。
- Redis execution-state 小 key 不被 bitmap / large-object 干扰。
- retry 自适应退避在下游异常时能自动保护主链路。
...
### 7.5 4000 readiness 指标

- per-lane worker pool 没有持续饥饿。
- 异步写 buffer 没有持续堆积。
- execution-state Redis p95 / p99 不受 route-cache 或 bitmap 影响。
- DAG cost profile 覆盖所有已发布画布。
- retry 自适应降速触发后，`LIGHT` / `STANDARD` RT 能恢复到目标阈值内。
