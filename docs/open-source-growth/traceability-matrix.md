...
| OSG-PLUGIN-001 | 插件注册必须复用现有 `PluginRegistryService` 路径 | `implementation-guardrails.md` 3.2 | Task 3 | `PluginRegistryService` 扩展 diff；无平行注册中心；`PluginRegistryServiceTest` | Month 2 |
| OSG-PLUGIN-002 | 插件 handler 必须兼容 `NodeHandler` 和 `@NodeHandlerType` | `implementation-guardrails.md` 3.3 | Task 3, Task 4 | `HandlerRegistry` 兼容测试；禁用插件后发布校验失败 | Month 2 |
...
- Final owner module: canvas-context-execution
